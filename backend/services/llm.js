...
const LLM_BASE = "https://api.dhenu.ai/v1"; // ✅ Dhenu API base
const LLM_KEY = process.env.LLM_API_KEY; // put your key in .env
const LLM_MODEL = process.env.LLM_MODEL || "dhenu2-in-8b-preview";

// Async generator → stream tokens from Dhenu API
async function* streamLLM(messages) {
  try {
    const resp = await axios({
      method: "post",
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a helpful farming assistant. Always reply in clear English.",
          },
          ...messages,
        ],
        stream: true,
    ...
    for await (const chunk of resp.data) {
      ...
      for (const line of parts) {
        ...
        try {
          const parsed = JSON.parse(data);
          const token = parsed.choices?.[0]?.delta?.content;
          if (token) yield token; // ✅ stream real pretrained tokens
        } catch (e) {
          console.error("Dhenu parse error:", e.message, line);
        }
      }
    }
  } catch (err) {
    console.error("Dhenu API error:", err.message || err);
    throw new Error("llm_failed");
  }
}
